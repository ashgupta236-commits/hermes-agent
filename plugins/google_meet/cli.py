...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    try:
        ...
        # `e` is captured to a string here because Python implicitly `del`s
        # the exception-as-target binding at the end of this except block,
        # so a closure referencing `e` directly would NameError once
        # argparse invokes _node_unavailable later.
        error_msg = str(e)

        def _node_unavailable(args):
            print(f"hermes meet node: module unavailable ({error_msg})")
            return 1
